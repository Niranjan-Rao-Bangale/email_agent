...
vectorstore = Pinecone.from_existing_index(index_name="new", embedding=embeddings, text_key="text",
                                           namespace="sales_docs2")
...
def search_orders(email):
    ...
    orders_df = pd.DataFrame({
        'email': ['b.niranjanrao@gmail.com'],
        'order_id': ['1234', '5678', '9012'],
        'customer': ['Joe Doe', 'Janet Doe', 'Babu Doe'],
...
orders_tool = Tool.from_function(
    name="Orders_DB",
    description="Use full when needing to look up some body orders with their email",
    func=search_orders
)

qa_tool = Tool.from_function(
    name="Business_docs",
    description="all of the documents and general info about the business - use full for answering general questions ",
    func=run_qa
)
...
3/ Structure your response in HTML with HTML tags for the line breaks
...
def researchAgent(query: Query):
    # query = query.query
    query = 'How to send a cold email to students for their next course'
